...
export default function MentalPage() {
  ...
  useEffect(() => {
    const fetchHabits = async () => {
      ...
      if (localUser) {
        const localHabits = getLocalHabits(localUser.id, "mental");
        const otherHabits = useUserStore.getState().habits.filter((h) => h.pillar !== "mental");
  ...
  const handleAddHabit = async (name: string, xp: number = 10) => {
    ...
    try {
      const localUser = getLocalSession();
      if (localUser) {
        addHabit(addLocalHabit(localUser.id, "mental", trimmedName, xp));

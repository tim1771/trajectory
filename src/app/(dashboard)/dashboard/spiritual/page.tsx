...
const HABIT_TEMPLATES = [
  { name: "Morning meditation (10 min)", icon: Sunrise, xp: 20 },
  { name: "Practice gratitude journaling", icon: Heart, xp: 15 },
  { name: "Reflect on core values", icon: Compass, xp: 15 },
  { name: "Spend time in nature", icon: TreePine, xp: 20 },
  { name: "Mindful breathing exercises", icon: Feather, xp: 10 },
];
...
export default function SpiritualPage() {
  ...
  const spiritualHabits = habits.filter((h) => h.pillar === "spiritual");
  const today = new Date().toISOString().split("T")[0];

  const completedToday = spiritualHabits.filter((h) =>
    h.completions?.some((c: any) => c.completed_at?.startsWith(today))
  ).length;

  const progress = spiritualHabits.length > 0
    ? Math.round((completedToday / spiritualHabits.length) * 100)
    : 0;
  ...
  useEffect(() => {
    const fetchHabits = async () => {
      ...
      if (localUser) {
        const localHabits = getLocalHabits(localUser.id, "spiritual");
        const otherHabits = useUserStore.getState().habits.filter((h) => h.pillar !== "spiritual");
  ...
  const handleAddHabit = async (name: string, xp: number = 10) => {
    ...
    try {
      const localUser = getLocalSession();
      if (localUser) {
        addHabit(addLocalHabit(localUser.id, "spiritual", trimmedName, xp));
        sound.success();
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-spiritual flex items-center justify-center">
              <Sparkles className="w-5 h-5 text-white" />
            </div>
            Spiritual Wellness
          </h1>
          <p className="text-white/60 mt-2">
            Discover meaning, purpose, and inner peace in your life
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <GlassCard className="flex items-center gap-6">
          <ProgressRing progress={progress} color="spiritual" />
          <div>
            <div className="text-2xl font-bold text-white">
              {completedToday}/{spiritualHabits.length}
            </div>
        ...
        <GlassCard>
          <div className="text-white/60 text-sm mb-1">Mindfulness Streak</div>
      ...
      {/* Habits List */}
      <GlassCard>
        <h2 className="text-xl font-semibold text-white mb-6">Your Spiritual Practices</h2>
        ...
        {spiritualHabits.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-white/60 mb-2">No spiritual habits yet</p>
            <p className="text-white/40 text-sm mb-6">
              Connect with your inner self and find deeper meaning
            </p>
            <GlassButton variant="primary" onClick={() => setShowAddModal(true)}>
              <Plus className="w-4 h-4 mr-1" />
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <GlassCard>
              <h2 className="text-xl font-semibold text-white mb-6">Add Spiritual Practice</h2>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {HABIT_TEMPLATES.map((template) => (
                    ...
                    >
                      <div className="w-10 h-10 rounded-lg bg-gradient-spiritual flex items-center justify-center flex-shrink-0">
...
function HabitRow({
  ...
  return (
    ...
    >
      ...
      <div className="text-violet-400 text-sm">+{habit.xpReward} XP</div>

...
const HABIT_TEMPLATES = [
  { name: "Call a friend or family member", icon: Phone, xp: 20 },
  { name: "Send an appreciation message", icon: Heart, xp: 10 },
  { name: "Have a meaningful conversation", icon: MessageCircle, xp: 15 },
  { name: "Plan a social activity", icon: Calendar, xp: 15 },
  { name: "Meet someone for coffee/lunch", icon: Coffee, xp: 25 },
];
...
export default function SocialPage() {
  ...
  const socialHabits = habits.filter((h) => h.pillar === "social");
  const today = new Date().toISOString().split("T")[0];

  const completedToday = socialHabits.filter((h) =>
    h.completions?.some((c: any) => c.completed_at?.startsWith(today))
  ).length;

  const progress = socialHabits.length > 0
    ? Math.round((completedToday / socialHabits.length) * 100)
    : 0;
  ...
  useEffect(() => {
    const fetchHabits = async () => {
      ...
      if (localUser) {
        const localHabits = getLocalHabits(localUser.id, "social");
        const otherHabits = useUserStore.getState().habits.filter((h) => h.pillar !== "social");
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-social flex items-center justify-center">
              <Users className="w-5 h-5 text-white" />
            </div>
            Social Wellness
          </h1>
          <p className="text-white/60 mt-2">
            Nurture meaningful relationships and build community connections
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <GlassCard className="flex items-center gap-6">
          <ProgressRing progress={progress} color="social" />
          <div>
            <div className="text-2xl font-bold text-white">
              {completedToday}/{socialHabits.length}
            </div>
        ...
        <GlassCard>
          <div className="text-white/60 text-sm mb-1">Connection Streak</div>
      ...
      {/* Habits List */}
      <GlassCard>
        <h2 className="text-xl font-semibold text-white mb-6">Your Social Habits</h2>

        {socialHabits.length === 0 ? (
          <div className="text-center py-12">
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-white/5 flex items-center justify-center">
              <Users className="w-8 h-8 text-white/40" />
            </div>
            <p className="text-white/60 mb-2">No social habits yet</p>
            <p className="text-white/40 text-sm mb-6">
              Build deeper connections with friends, family, and community
            </p>
            <GlassButton variant="primary" onClick={() => setShowAddModal(true)}>
              <Plus className="w-4 h-4 mr-1" />
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <GlassCard>
              <h2 className="text-xl font-semibold text-white mb-6">Add Social Habit</h2>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {HABIT_TEMPLATES.map((template) => (
                    ...
                    >
                      <div className="w-10 h-10 rounded-lg bg-gradient-social flex items-center justify-center flex-shrink-0">
...
function HabitRow({
  ...
  return (
    ...
    >
      ...
      <div className="text-orange-400 text-sm">+{habit.xpReward} XP</div>

...
const HABIT_TEMPLATES = [
  { name: "Reduce, reuse, or recycle something", icon: Recycle, xp: 15 },
  { name: "Declutter or organize a space", icon: Home, xp: 15 },
  { name: "Spend 20 min outdoors in nature", icon: Sun, xp: 20 },
  { name: "Conserve water (shorter shower)", icon: Droplet, xp: 10 },
  { name: "Care for a plant or garden", icon: TreePine, xp: 15 },
];
...
export default function EnvironmentalPage() {
  ...
  const environmentalHabits = habits.filter((h) => h.pillar === "environmental");
  const today = new Date().toISOString().split("T")[0];

  const completedToday = environmentalHabits.filter((h) =>
    h.completions?.some((c: any) => c.completed_at?.startsWith(today))
  ).length;

  const progress = environmentalHabits.length > 0
    ? Math.round((completedToday / environmentalHabits.length) * 100)
    : 0;
  ...
  useEffect(() => {
    const fetchHabits = async () => {
      ...
      if (localUser) {
        const localHabits = getLocalHabits(localUser.id, "environmental");
        const otherHabits = useUserStore.getState().habits.filter((h) => h.pillar !== "environmental");
  ...
  const handleAddHabit = async (name: string, xp: number = 10) => {
    ...
    try {
      const localUser = getLocalSession();
      if (localUser) {
        addHabit(addLocalHabit(localUser.id, "environmental", trimmedName, xp));
        sound.success();
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-environmental flex items-center justify-center">
              <Leaf className="w-5 h-5 text-white" />
            </div>
            Environmental Wellness
          </h1>
          <p className="text-white/60 mt-2">
            Create harmony with your surroundings and embrace sustainability
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <GlassCard className="flex items-center gap-6">
          <ProgressRing progress={progress} color="environmental" />
          <div>
            <div className="text-2xl font-bold text-white">
              {completedToday}/{environmentalHabits.length}
            </div>
        ...
        <GlassCard>
          <div className="text-white/60 text-sm mb-1">Eco Streak</div>
        ...
        <GlassCard>
          ...
          <div className="text-3xl font-bold text-teal-400">
            {environmentalHabits.reduce((sum, h) => {
      ...
      {/* Habits List */}
      <GlassCard>
        <h2 className="text-xl font-semibold text-white mb-6">Your Environmental Habits</h2>

        {environmentalHabits.length === 0 ? (
          <div className="text-center py-12">
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-white/5 flex items-center justify-center">
              <Leaf className="w-8 h-8 text-white/40" />
            </div>
            <p className="text-white/60 mb-2">No environmental habits yet</p>
            <p className="text-white/40 text-sm mb-6">
              Nurture your connection with nature and your surroundings
            </p>
            <GlassButton variant="primary" onClick={() => setShowAddModal(true)}>
              <Plus className="w-4 h-4 mr-1" />
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <GlassCard>
              <h2 className="text-xl font-semibold text-white mb-6">Add Environmental Habit</h2>
...
function HabitRow({
  ...
  return (
    ...
    >
      ...
      <div className="text-teal-400 text-sm">+{habit.xpReward} XP</div>

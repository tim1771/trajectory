...
const HABIT_TEMPLATES = [
  { name: "Complete top 3 priority tasks", icon: Target, xp: 25 },
  { name: "Take proper lunch break (away from desk)", icon: Clock, xp: 15 },
  { name: "Learn a new work-related skill", icon: TrendingUp, xp: 20 },
  { name: "Network or connect with a colleague", icon: Users, xp: 15 },
  { name: "Celebrate a work accomplishment", icon: Award, xp: 10 },
];
...
export default function OccupationalPage() {
  ...
  const occupationalHabits = habits.filter((h) => h.pillar === "occupational");
  const today = new Date().toISOString().split("T")[0];

  const completedToday = occupationalHabits.filter((h) =>
    h.completions?.some((c: any) => c.completed_at?.startsWith(today))
  ).length;

  const progress = occupationalHabits.length > 0
    ? Math.round((completedToday / occupationalHabits.length) * 100)
    : 0;
  ...
  useEffect(() => {
    const fetchHabits = async () => {
      ...
      if (localUser) {
        const localHabits = getLocalHabits(localUser.id, "occupational");
        const otherHabits = useUserStore.getState().habits.filter((h) => h.pillar !== "occupational");
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-occupational flex items-center justify-center">
              <Briefcase className="w-5 h-5 text-white" />
            </div>
            Occupational Wellness
          </h1>
          <p className="text-white/60 mt-2">
            Find fulfillment and achieve work-life balance
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <GlassCard className="flex items-center gap-6">
          <ProgressRing progress={progress} color="occupational" />
          <div>
            <div className="text-2xl font-bold text-white">
              {completedToday}/{occupationalHabits.length}
            </div>
        ...
        <GlassCard>
          <div className="text-white/60 text-sm mb-1">Productivity Streak</div>
        ...
        <GlassCard>
          ...
          <div className="text-3xl font-bold text-green-400">
            {occupationalHabits.reduce((sum, h) => {
      ...
      {/* Habits List */}
      <GlassCard>
        <h2 className="text-xl font-semibold text-white mb-6">Your Work Habits</h2>

        {occupationalHabits.length === 0 ? (
          <div className="text-center py-12">
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-white/5 flex items-center justify-center">
              <Briefcase className="w-8 h-8 text-white/40" />
            </div>
            <p className="text-white/60 mb-2">No occupational habits yet</p>
            <p className="text-white/40 text-sm mb-6">
              Build habits for career growth and work-life balance
            </p>
            <GlassButton variant="primary" onClick={() => setShowAddModal(true)}>
              <Plus className="w-4 h-4 mr-1" />
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <GlassCard>
              <h2 className="text-xl font-semibold text-white mb-6">Add Work Habit</h2>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {HABIT_TEMPLATES.map((template) => (
                    ...
                    >
                      <div className="w-10 h-10 rounded-lg bg-gradient-occupational flex items-center justify-center flex-shrink-0">
...
function HabitRow({
  ...
  return (
    ...
    >
      ...
      <div className="text-green-400 text-sm">+{habit.xpReward} XP</div>
